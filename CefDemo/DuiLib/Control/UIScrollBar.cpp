...
namespace DuiLib
{
	CScrollBarUI::CScrollBarUI() : m_bHorizontal(false), m_nRange(100), m_nScrollPos(0), m_nLineSize(8), 
		...
		m_uButton2State(0), m_uThumbState(0), m_bShowButton1(true), m_bShowButton2(true),sink_(nullptr),mouse_enter_flag_(false),mouse_click_flag_(false),move_sink_(nullptr)
	{
	...
	void CScrollBarUI::SetSink(IMouseSlot* sink)
	{
		sink_ = sink;
	}

	bool CScrollBarUI::IsMouseEnter()
	{
		return mouse_enter_flag_;
	}

	bool CScrollBarUI::IsMouseClick() 
	{
		return mouse_click_flag_;
	}

	void CScrollBarUI::SetMoveSink(IScrollBarSlot* sink)
	{
		move_sink_ = sink;
	}
	...
	void CScrollBarUI::SetScrollPos(int nPos)
	{
		...
		if(move_sink_) move_sink_->MoveEvent(nPos);
	...
	void CScrollBarUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
		{
			...
			if(sink_) sink_->MouseEvent(mouse_enter_flag_, mouse_click_flag_);
			if( !IsEnabled() ) return;
