...
namespace DuiLib
{
	...
	int COptionUI::GetSelectedIndexInGroup()
	{
		int iReturn = 0;
		CStdPtrArray* aOptionGroup = m_pManager->GetOptionGroup(m_sGroupName);
		for( int i = 0; i < aOptionGroup->GetSize(); i++ )
	...
	void COptionUI::SetSelectedIndexInGroup(int index)
	{
		CStdPtrArray* aOptionGroup = m_pManager->GetOptionGroup(m_sGroupName);		
		COptionUI* pControl = static_cast<COptionUI*>(aOptionGroup->GetAt(index));
		if( pControl != NULL ) 
		{				
