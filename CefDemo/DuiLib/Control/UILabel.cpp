...
namespace DuiLib
{
	...
	SIZE CLabelUI::GetTextSize()
	{
		return CRenderEngine::GetTextSize(m_pManager->GetPaintDC(), m_pManager, m_sText, GetFont(), m_uTextStyle);		
	}

	int CLabelUI::GetAllWidth()
	{
		int iWidth;
		iWidth = 2*GetBorderSize();
		iWidth += 2*GetBorderRound().cx;
		iWidth += GetTextPadding().left;
		iWidth += GetTextPadding().right;
		iWidth += GetTextSize().cx;
		return iWidth;
	}
}