...
void CRichEditUI::SetText(LPCTSTR pstrText)
{
	...
	{
		m_sText = m_sHintText;
		m_pTwh->SetColor(m_dwHintTextColor);
	}
	else
	{
		m_sText = pstrText;
...
void CRichEditUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_SETFOCUS ) {
		...
		m_bFocusInput = true;
		m_bFocused = true;
		if (m_bFocusInput) m_pTwh->SetColor(m_dwTextColor);
		else m_pTwh->SetColor(m_dwHintTextColor);
...
void CRichEditUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	...
	if (bFirstPaint && !m_bFocusInput && m_sHintText != _T(""))
	{
		bFirstPaint = false;
...
void CRichEditUI::AutoFitContent()
{
	SIZE size;
	size = CRenderEngine::GetTextSize(m_pManager->GetPaintDC(), m_pManager, m_sText, GetFont(), m_lTwhStyle);
	int iFixWidth = GetFixedWidth() - 10;
	int iLines = size.cx/iFixWidth + 1;
	int iHeight = size.cy * iLines;
	SetFixedHeight(iHeight);
}

void CRichEditUI::SetHintText(LPCTSTR pstrText) 
{
	m_sHintText = pstrText;
	SetTextInternal(m_sHintText);
}

CDuiString CRichEditUI::GetHintText() const 
{
	return m_sHintText;
}

void CRichEditUI::SetTextInternal(LPCTSTR pstrText)
{
	m_sText = pstrText;
