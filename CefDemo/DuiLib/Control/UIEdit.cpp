...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if( uMsg == WM_KILLFOCUS ) 
		{
			...
			if (IsWindow(GetHWND())) 
			{
				if (m_pOwner != NULL && m_pOwner->GetPaintManager() != NULL)
				{
					HWND mainWnd = m_pOwner->GetPaintManager()->GetPaintWindow();
					HWND focusWnd = ::GetFocus();
					if (focusWnd != mainWnd) {
						::PostMessageA(mainWnd, WM_KILLFOCUS, 0, 0);
						m_pOwner->DoEvent(event);
						m_pOwner->GetPaintManager()->SetFocus(NULL);
					} else {
						if (m_pOwner != m_pOwner->GetPaintManager()->GetFocus())
							m_pOwner->DoEvent(event);
					} 
				}
			}
	...
	LRESULT CEditWnd::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if (m_pOwner->m_bFocusInput)	//只发送用户手动输入时的消息
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_iWindowStyls(0),m_bFocusInput(false)
	{
		m_bPasswordModeKeep = false;
	...
	void CEditUI::DoEvent(TEventUI& event)
	{
		...
		{	
			SetPasswordMode(m_bPasswordModeKeep);
			if( m_pWindow ) return;
			m_pWindow = new CEditWnd();
			ASSERT(m_pWindow);
			m_pWindow->Init(this);
			if (!m_bFocusInput /*&& GetText() == GetHintText()*/)
			{
				SetTextInternal(_T(""));				
			}
			...
			SetSel(IsPasswordMode() ? nSize : 0,nSize);
		...
		{
			if (CLabelUI::GetText() == _T(""))
				m_bFocusInput = false;

			if (!m_bFocusInput)
			{
				SetPasswordMode(0);
				SetTextInternal(GetHintText());					
			}		
			else
			{
				SetPasswordMode(m_bPasswordModeKeep);
		...
		if (event.Type == UIEVENT_CHANGE_LANGUAGE_VERSION_EDIT)
		{
			CDuiString strTemp;
			CDuiString text = CPaintManagerUI::GetLanguage(event.pControlID);
			strTemp.Format(text, event.pControlInfo);
			if(event.dwControlType == TYPE_EDIT_HINT) m_sHintText = strTemp;
			SetPasswordMode(0);
			SetTextInternal(m_sHintText);
		}
	...
	{
		m_bFocusInput = _tcslen(pstrText) > 0 ? true : false;
		if (!m_bFocusInput && m_sHintText != _T(""))
		{
			SetPasswordMode(0);
			SetTextInternal(m_sHintText);
		}
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("hint")) == 0 ) SetHintText(pstrValue);
		else if( _tcscmp(pstrName, _T("hintid")) == 0 ) SetHintTextById(pstrValue);
		...
		else if( _tcscmp(pstrName, _T("hinttextcolor")) == 0 ) {
			if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
			m_dwHintTextColor = clrColor;
		}
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		if( m_dwHintTextColor == 0) m_dwHintTextColor = m_pManager->GetDefaultFontColor();
		...
		if( IsEnabled() ) {
			//增加设置hint文本颜色代码
			if (!m_bFocusInput && m_sHintText != _T(""))
				CRenderEngine::DrawText(hDC, m_pManager, rc, sText, m_dwHintTextColor, \
	...
	{
		m_sHintText = pstrText;	
		SetPasswordMode(0);
		SetTextInternal(m_sHintText);
	}

	void CEditUI::SetHintTextById(LPCTSTR pID)
	{
		CDuiString text = CPaintManagerUI::GetLanguage(pID);
		m_sHintText = text;
		m_pManager->AddControl(this, TYPE_EDIT_HINT, pID);
		SetPasswordMode(0);
		SetTextInternal(m_sHintText);
	}

	CDuiString	CEditUI::GetHintText() const
	{
		return m_sHintText;
	}
	...
	void CEditUI::SetTextInternal(LPCTSTR pstrText) 
	{
		m_sText = pstrText;
		if( m_pWindow != NULL ) Edit_SetText(*m_pWindow, m_sText);
		Invalidate();
	...
	CPaintManagerUI* CEditUI::GetPaintManager()
	{
		return m_pManager;
	}
