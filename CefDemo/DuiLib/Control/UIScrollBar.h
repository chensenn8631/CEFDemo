...
namespace DuiLib
{
	struct IMouseSlot
	{
		virtual void MouseEvent(bool enter, bool click) = 0;
	};

	struct IScrollBarSlot
	{
		virtual void MoveEvent(int pos) = 0;
	};
	...
	public:
		...
		void SetSink(IMouseSlot* sink);
		bool IsMouseEnter();
		bool IsMouseClick();

		void SetMoveSink(IScrollBarSlot* sink);
	...
	private:
		IScrollBarSlot* move_sink_;
		IMouseSlot* sink_;
		bool mouse_enter_flag_;
		bool mouse_click_flag_;
	};
}
