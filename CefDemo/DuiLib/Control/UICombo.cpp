...
void CComboWnd::Init(CComboUI* pOwner)
{
	...
	if (iVisible == 0)
	{
		...
		Create(pOwner->GetManager()->GetPaintWindow(), NULL, WS_POPUP | WS_DISABLED, WS_EX_TOOLWINDOW, rc);
...
//      m_pOwner->SetFocus();
		...
		if (m_pOwner->GetComboWndDisableMsg(_T("WM_LBUTTONUP")) == NULL) 
		{
			POINT pt = { 0 };
			::GetCursorPos(&pt);
		...
		if (m_pOwner->GetComboWndDisableMsg(_T("WM_KEYDOWN")) == NULL)
		{
			switch( wParam ) {
			case VK_ESCAPE:
		...
		if (m_pOwner->GetComboWndDisableMsg(_T("WM_MOUSEWHEEL")) == NULL)
		{
...
void CComboUI::SetComboWndDisableMsg(LPCTSTR msg)
{
	m_sDisableMsg.Insert(msg, "true");
}

PVOID CComboUI::GetComboWndDisableMsg(LPCTSTR msg)
{
	return m_sDisableMsg.Find(msg);
}

HWND CComboUI::GetComboWndHwnd() const 
{
	if (m_pWindow != NULL)
		return m_pWindow->m_pm.GetPaintWindow();
