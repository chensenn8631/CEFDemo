...
//Ö§³Ö¶àÓïÑÔ
CStdPtrArray m_aChanger;
boost::property_tree::ptree m_ptreeLanguage;
LPCTSTR m_pChineseLanguagePath = NULL;
LPCTSTR m_pEnglishLanguagePath = NULL;
...
bool CPaintManagerUI::SetLanguageEnvironment(LPCTSTR pCnPath, LPCTSTR pEnPath, bool bDefaultLanguage)
{
	if (pCnPath == _T("") || pEnPath == _T("")) return false;
	if (bDefaultLanguage) {
		if(_taccess_s(pCnPath, 0) != 0) return false;
	}
	else {
		if(_taccess_s(pEnPath, 0) != 0) return false;
	}
	m_pChineseLanguagePath = pCnPath;
	m_pEnglishLanguagePath = pEnPath; 
	boost::property_tree::read_xml(bDefaultLanguage ? CW2A(pCnPath, CP_ACP).m_psz : CW2A(pEnPath, CP_ACP).m_psz, m_ptreeLanguage);
	return true;
}
...
//#endif
    ...
    while( pMsg = static_cast<TNotifyUI*>(m_aAsyncNotify.GetAt(0)) ) {
        ...
        if( pMsg->pSender != NULL ) 
		{
			...
			if( pMsg->pSender->OnNotifyUI ) pMsg->pSender->OnNotifyUI(pMsg);			
        }
    ...
    case WM_TIMER:
        ...
        break;
	case WM_CHANGE_lANGUAGE_VERSION:
		{
			if (lParam == 2052)
			{
				boost::property_tree::read_xml(CW2A(m_pChineseLanguagePath).m_psz, m_ptreeLanguage);
			}
			else if(lParam == 1033)
			{
				boost::property_tree::read_xml(CW2A(m_pEnglishLanguagePath).m_psz, m_ptreeLanguage);
			}
			for( int i = 0; i < m_aChanger.GetSize(); i++ ) {
				const CONTROLINFO* pInfo = static_cast<CONTROLINFO*>(m_aChanger[i]);
/*				if( pInfo->hWnd == m_hWndPaint) {*/
					TEventUI event = { 0 };
					if (pInfo->nType == TYPE_BTN_HOTIMAGE ||
						pInfo->nType == TYPE_BTN_NORMALIMAGE ||
						pInfo->nType == TYPE_BTN_PUSHIMAGE)
						event.Type = UIEVENT_CHANGE_lANGUAGE_VERSION_BUTTON;
					else if (pInfo->nType == TYPE_EDIT_HINT)
						event.Type = UIEVENT_CHANGE_LANGUAGE_VERSION_EDIT;
					else
						event.Type = UIEVENT_CHANGE_lANGUAGE_VERSION;
					event.pSender = pInfo->pSender;
					event.dwControlType = pInfo->nType;
					event.pControlInfo = pInfo->strInfo.c_str();
					event.pControlID = pInfo->strID.c_str();
					event.wParam = wParam;
					event.lParam = lParam;
					pInfo->pSender->Event(event);
					continue;
/*				}*/
			}
		}
    ...
    case WM_LBUTTONDOWN:
        {
			...
			if(m_hwndTooltip) ::SendMessage(m_hwndTooltip, TTM_DELTOOL, 0, (LPARAM) &m_ToolTip);
...
bool CPaintManagerUI::AddControl(CControlUI* pControl, ControlType nControlType, LPCTSTR pControlID, LPCTSTR info)
{
	ASSERT(pControl!=NULL);
	CONTROLINFO* pInfo = new CONTROLINFO;
	pInfo->hWnd = pControl->GetManager()->GetPaintWindow();
	pInfo->pSender = pControl;
	pInfo->strID = pControlID;
	pInfo->nType = nControlType;
	pInfo->strInfo = info;
	return m_aChanger.Add(pInfo);
}

void CPaintManagerUI::RemoveControl(HWND hWnd)
{
	ASSERT(hWnd!=NULL);
	for( int i = 0; i < m_aChanger.GetSize(); i++ ) {
		if(static_cast<CONTROLINFO*>(m_aChanger[i])->hWnd == hWnd) {
			m_aChanger.Remove(i);
		}
	}
}
...
void CPaintManagerUI::SendNotify(TNotifyUI& Msg, bool bAsync /*= false*/)
{
    ...
    if( !bAsync ) {
        ...
        if( Msg.pSender != NULL ) 
		{
			...
			if( Msg.pSender->OnNotifyUI ) Msg.pSender->OnNotifyUI(&Msg);
		}
        for( int i = 0; i < m_aNotifiers.GetSize(); i++ ) {
...
CDuiString CPaintManagerUI::GetLanguage(LPCTSTR id)
{
	return CDuiString (CA2W( m_ptreeLanguage.get<std::string>(CW2A(id).m_psz, "").c_str(), CP_UTF8));
}
