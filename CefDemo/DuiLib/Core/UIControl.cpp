...
void CControlUI::SetTextById(LPCTSTR pID, LPCTSTR pstrText)
{
	CDuiString strTemp;
	CDuiString text = CPaintManagerUI::GetLanguage(pID);
	strTemp.Format(text, pstrText);
	m_sText = strTemp;
	m_pManager->AddControl(this, TYPE_TEXT, pID, pstrText);
	Invalidate();
}
...
void CControlUI::SetBKImageById(LPCTSTR pID)
{
	CDuiString bk_image = CPaintManagerUI::GetLanguage(pID);
	m_sBkImage = bk_image;
	m_pManager->AddControl(this, TYPE_BKIMAGE, pID);
	Invalidate();
}
...
void CControlUI::SetTooltipById(LPCTSTR pID, LPCTSTR pstrText)
{
	CDuiString strTemp;
	CDuiString text = CPaintManagerUI::GetLanguage(pID);
	strTemp.Format(text, pstrText);
	m_sToolTip = strTemp;
	m_pManager->AddControl(this, TYPE_TIP, pID, pstrText);
	Invalidate();
}
...
void CControlUI::DoEvent(TEventUI& event)
{
	...
	if (event.Type == UIEVENT_CHANGE_lANGUAGE_VERSION)
	{
		CDuiString strTemp;
		CDuiString text = CPaintManagerUI::GetLanguage(event.pControlID);
		strTemp.Format(text, event.pControlInfo);
		if(event.dwControlType == TYPE_TEXT) m_sText = strTemp;
		else if(event.dwControlType == TYPE_TIP) m_sToolTip = strTemp;
		else if(event.dwControlType == TYPE_BKIMAGE) m_sBkImage = strTemp;
		Invalidate();
	}
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("bkimageid")) == 0 ) SetBKImageById(pstrValue);
	...
	else if( _tcscmp(pstrName, _T("textid")) == 0 ) SetTextById(pstrValue);
    else if( _tcscmp(pstrName, _T("tooltip")) == 0 ) SetToolTip(pstrValue);
	else if( _tcscmp(pstrName, _T("tooltipid")) == 0 ) SetTooltipById(pstrValue);
