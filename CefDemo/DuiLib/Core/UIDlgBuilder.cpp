    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 10:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_BUTTON_HTML) == 0 )       pControl = new CButtonHtmlUI;
                break;
			case 11:
				if (_tcscmp(pstrClass, DUI_CTR_CHILDLAYOUT) == 0)			  pControl = new CChildLayoutUI;
				else if( _tcscmp(pstrClass, DUI_CTR_BUTTON_ARROW) == 0 )      pControl = new CButtonArrowUI;
				else if( _tcscmp(pstrClass, DUI_CTR_OPTION_ARROW) == 0 )      pControl = new COptionArrowUI;
				break;
			case 13:
				if( _tcscmp(pstrClass, DUI_CTR_CHECKBOX_ARROW) == 0 )    pControl = new CCheckBoxArrowUI;
				break;
            case 14:
