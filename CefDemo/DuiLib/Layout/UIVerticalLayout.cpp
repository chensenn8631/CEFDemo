...
namespace DuiLib
{
	...
	void CVerticalLayoutUI::SetPos(RECT rc)
	{
		...
		int cyFixed = 0;
		int nEstimateNum = 0;
		int nLastBottomPadding = 0x80000000;
		for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
			nLastBottomPadding = 0x80000000;
			...
			cyFixed += sz.cy + MAX(pControl->GetPadding().top, nLastBottomPadding);
			nEstimateNum++;
			nLastBottomPadding = pControl->GetPadding().bottom;
			if (it1 == m_items.GetSize() - 1) {
				cyFixed += nLastBottomPadding;
			}
		}
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			int iPadding = MAX(rcPadding.top, nLastBottomPadding);
			szRemaining.cy -= iPadding;
			cyFixedRemaining -= iPadding;
			...
			RECT rcCtrl = { iPosX + rcPadding.left, iPosY + iPadding, iPosX + rcPadding.left + sz.cx, iPosY + sz.cy + iPadding};
			pControl->SetPos(rcCtrl);

			iPosY += sz.cy + m_iChildPadding + iPadding;
			cyNeeded += sz.cy + iPadding;
			szRemaining.cy -= sz.cy + m_iChildPadding;
			cyFixedRemaining -= m_iChildPadding;
			nLastBottomPadding = rcPadding.bottom;
		}
