...
namespace DuiLib
{
	...
	void CHorizontalLayoutUI::SetPos(RECT rc)
	{
		...
		int cxFixed = 0;
		int nEstimateNum = 0;
		int nLastRightPadding = 0x80000000;
		for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
			nLastRightPadding = 0x80000000;
			...
			cxFixed += sz.cx +  MAX(pControl->GetPadding().left, nLastRightPadding);
			nEstimateNum++;
			nLastRightPadding = pControl->GetPadding().right;
			if (it1 == m_items.GetSize() - 1) {
				cxFixed += nLastRightPadding;
			}
		}
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			int iPadding = MAX(rcPadding.left, nLastRightPadding);
			szRemaining.cx -= iPadding;
			cxFixedRemaining -= iPadding;
			...
			RECT rcCtrl = { iPosX + iPadding, rc.top + rcPadding.top, iPosX + sz.cx + iPadding, rc.top + rcPadding.top + sz.cy};
			pControl->SetPos(rcCtrl);
			iPosX += sz.cx + m_iChildPadding + iPadding;
            cxNeeded += sz.cx + iPadding;
			szRemaining.cx -= sz.cx + m_iChildPadding;
			cxFixedRemaining -= m_iChildPadding;
			nLastRightPadding = rcPadding.right;
		}
